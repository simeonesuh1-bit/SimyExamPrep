...
async function uploadTricky() {
  console.log(`Starting upload of ${finalTricky.length} tricky questions...`);
  const batch = writeBatch(db);
  
  finalTricky.forEach(q => {
    const qId = `tricky_${q.course}_${q.id}`;
    const qRef = doc(db, 'questions', qId);
    batch.set(qRef, {
      ...q,
      id: qId,
      source: 'tricky_expansion_manual'
    });
  });

  await batch.commit();
  console.log('✅ Successfully uploaded tricky questions to Firestore!');
}

uploadTricky().catch(console.error);
